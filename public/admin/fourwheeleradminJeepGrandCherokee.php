...
if ( isset($_GET['edit']) ) {
 	...
 	$fetchCarRecords = $conn->query(" SELECT * FROM fourwheel WHERE FourwheelName='Grand Cherokee' ") or die($conn->error) ;
...
if ( isset($_POST['submitUpdate']) ) {
	...
	if ( empty($brandErr) && empty($carNameErr) && empty($modelcolorErr) && empty($modelYearErr) && empty($modelFuelTypeErr) && empty($modelSittingCapacityErr) && empty($modelEngineCylindersErr) && empty($modelSteeringErr) && empty($modelDriveModesErr) && empty($modelTransmissionErr) && empty($modelGearboxErr) && empty($modelWheelDriveErr) && empty($modelPowerSteeringErr) && empty($modelSpareWheelErr) && empty($modelHorsepowerErr) && empty($modelHybridErr) && empty($modelPriceErr) ) {
		...
		if ( $conn->query($updateFourwheel) === TRUE ) {   
			$_SESSION['recUpdated'] ;
			$_SESSION['classTypeAccept'] ;
			header('location: fourwheeleradminJeepGrandCherokee.php?recUpdt') ;
		} else {
			$_SESSION['recFail'] ;
			$_SESSION['classTypeError'] ;
			header('location: fourwheeleradminJeepGrandCherokee.php?recF') ;
		}
	}
		...
		<div class="col-md-9">
			...
			<div class="col-md-6">
				...
				?> w-50 text-center text-success text-nowrap font-weight-bold mx-auto" >
				 ...
				 <?php
				 	$fetchCarRecords = " SELECT * FROM fourwheel WHERE FourwheelName='Grand Cherokee' " ;
				...
				<form class="form" action="fourwheeleradminJeepGrandCherokee.php" method="post" >  
					...
					<div class="form-group">
						<div class="col-md">
							...
							<div class="row">
								<div class="col">
									<a href="fourwheeleradminJeepGrandCherokee.php?edit=<?php echo $rowFetch['id'] ; ?>" class="form-control form-control-lg btn btn-lg bg-info">Edit</a>
								</div>
								<div class="col">
									<a href="fourwheeleradminJeepGrandCherokee.php?delete=<?php echo $rowFetch['id'] ; ?>" class="form-control form-control-lg btn btn-lg bg-danger" onclick="confirm_change()">Delete</a>
								</div>
							</div>	
							<?php endif ; ?>
						</div>
